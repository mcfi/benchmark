...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Ben N");
MODULE_DESCRIPTION("Linux kernel module to microbenchmark memory copy between user and kernel");
...
static __init int ku_copy_benchmark(void)
{
	...
	pr_info("copy_to_user   %8u bytes: %10llu ns (%10u iters) %14llu bytes/s\n",
               size, end - start, iterations, iterations * size * 1000000000UL / (end - start));
